  ...
  <form @submit.prevent="handleFormSubmit" class="flex flex-col gap-2">
    ...
    <div v-for="(item, index) in fields" class="border-b py-1 flex flex-col md:flex-row">
        ...
        <div class="md:flex-1">
          ...
          <Select v-else-if="item.type == 'select'" v-model="form[item.key]" :options="optionPendapatan" optionLabel="label" optionValue="value" placeholder="Pilih" class="w-full" />
...
const fields = [
  ...
  { key: 'nama', label: 'Nama', type: 'text', desc: 'Nama Akun Biaya' },
  { key: 'akunpendapatan_id', label: 'Sumber Pendapatan', type: 'select', desc: 'Pilih Akun pendapatan' },
]
...
if(data){
  form.id = data.id
  form.nama = data.nama
  form.akunpendapatan_id = data.pendapatan_id
}
...
const handleFormSubmit = async () => {  
  ...
  if(action=='edit') {
    try {
      const res = await client('/api/akunpengeluaran/'+data.id, { method: 'PUT', body: form });
    ...
    try {
      const res = await client('/api/akunpengeluaran', { method: 'POST', body: form });
...
const { data: optionPendapatan ,refresh: fPendapatan } = await useAsyncData(
    'option-pendapatan',
    () => client('/api/option/akunpendapatan')
)
